...
import {
  ...
  type FuelTypeSheet,
  type GWP,
} from "../../../web/schema/mobileEmissionSchema";
import { db } from "../db";
...
export const mobileEmissionsRouter = new Hono()
  .get("/results", async (c) => {
    ...
    const data = mobileEmissionsData.map((row) => {
  ...
  .get("/many", zValidator("json", paginationSchema), async (c) => {
    const { skip, take } = c.req.valid("json");

    const mobileEmissionsData = await db.select().from(mobileEmissions).limit(take).offset(skip);
